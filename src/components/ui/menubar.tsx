...
function Menubar({
  ...
  return (
    <MenubarPrimitive.Root
      data-slot="menubar"
      className={cn(
        "flex h-8 items-center gap-0.5 rounded-none border border-foreground/40 bg-card p-[3px]",
        className
      )}
      {...props}
    />
...
function MenubarTrigger({
  ...
  return (
    <MenubarPrimitive.Trigger
      ...
      className={cn(
        "flex items-center rounded-none px-2 py-[2px] font-condensed text-xs font-medium tracking-[0.1em] uppercase outline-hidden select-none hover:bg-muted aria-expanded:bg-muted",
        className
      )}
      {...props}
    />
  ...
  return (
    <MenubarPortal>
      <MenubarPrimitive.Content
        ...
        className={cn("z-50 min-w-36 origin-(--radix-menubar-content-transform-origin) overflow-hidden rounded-lg bg-popover p-1 text-popover-foreground shadow-[3px_3px_0_0_--alpha(var(--foreground)/15%)] ring-1 ring-foreground/50 duration-100 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95", className )}
        {...props}
      />
    </MenubarPortal>
  )
